%random number generator for replication
rng(111);


%First Parameterization
mu = [4, 3.5, 2.5, 2.8, 4.5];
sigma = [.42250, .16900, .04225, .01521, .11430;
    .16900, .25532, .27056, .02435, .06764;
    .04225, .27056, .67640, .10000, .00900;
    .01521, .02435, .10000 , 1.0000, .03600;
    .11430, .06764, .00900, .03600, .59000];
n = 100000;
...
%Consumer Welfare
firstcolumn = agg;
secondcolumn = ones(500000, 1) * pmax;;
matrix = [firstcolumn, secondcolumn];
matrix(:, 3) = matrix(:, 1) - matrix(:, 2);
matrix = matrix(matrix(:, 3) >= 0, :); 
welfare = sum(matrix(:, 3));
...
%Consumer Welfare
bfirstcolumn = bagg;
bsecondcolumn = ones(100000, 1) * bpmax;;
bmatrix = [bfirstcolumn, bsecondcolumn];
bmatrix(:, 3) = bmatrix(:, 1) - bmatrix(:, 2);
bmatrix = bmatrix(bmatrix(:, 3) >= 0, :); 
bwelfare = sum(bmatrix(:, 3));
...
%Welfare
firstcolumn1 = data1;
secondcolumn1 = ones(100000,1) * pmax1;
matrix1 = [firstcolumn1, secondcolumn1];
matrix1(:, 3) = matrix1(:, 1) - matrix1(:, 2);
matrix1 = matrix1(matrix1(:, 3) >= 0, :); 
welfare1 = sum(matrix1(:, 3));
...
%Welfare
firstcolumn2 = data2;
secondcolumn2 = ones(100000,1) * pmax2;
matrix2 = [firstcolumn2, secondcolumn2];
matrix2(:, 3) = matrix2(:, 1) - matrix2(:, 2);
matrix2 = matrix2(matrix2(:, 3) >= 0, :); 
welfare2 = sum(matrix2(:, 3));
...
%Welfare
firstcolumn3 = data3;
secondcolumn3 = ones(100000,1) * pmax3;
matrix3 = [firstcolumn3, secondcolumn3];
matrix3(:, 3) = matrix3(:, 1) - matrix3(:, 2);
matrix3 = matrix3(matrix3(:, 3) >= 0, :); 
welfare3 = sum(matrix3(:, 3));
...
%Welfare
firstcolumn4 = data4;
secondcolumn4 = ones(100000,1) * pmax4;
matrix4 = [firstcolumn4, secondcolumn4];
matrix4(:, 3) = matrix4(:, 1) - matrix4(:, 2);
matrix4 = matrix4(matrix4(:, 3) >= 0, :); 
welfare4 = sum(matrix4(:, 3));
...
%Welfare
firstcolumn5 = data5;
secondcolumn5 = ones(100000,1) * pmax5;
matrix5 = [firstcolumn5, secondcolumn5];
matrix5(:, 3) = matrix5(:, 1) - matrix5(:, 2);
matrix5 = matrix5(matrix5(:, 3) >= 0, :); 
welfare5 = sum(matrix5(:, 3));  

totalwelfare= welfare1 + welfare2 + welfare3 + welfare4 + welfare5;

hlink = uihyperlink(IO_PS3.m);